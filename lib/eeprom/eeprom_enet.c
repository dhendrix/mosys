 ...
 *
 * eeprom_enet: mosys ethernet EEPROM routines, based off of ethtool.c, the 
 * Linux ethernet device configuration tool.  This file interfaces with the
 * kernel driver to do eeprom reads/writes.
 */
#include <sys/types.h>
#include <string.h>
#include <stdlib.h>
...
static enum {
	...
	EEPROM_ENET_GPAUSE,
	EEPROM_ENET_SPAUSE,
	EEPROM_ENET_GCOALESCE,
	EEPROM_ENET_SCOALESCE,
	EEPROM_ENET_GRING,
	EEPROM_ENET_SRING,
	EEPROM_ENET_GOFFLOAD,
	EEPROM_ENET_SOFFLOAD,
	EEPROM_ENET_GSTATS,
} mode = EEPROM_ENET_GSET;
#endif
...
static struct option {
    ...
    { "-s", "--change", EEPROM_ENET_SSET, "Change generic options",
		...
		"		[ sopass %%x:%%x:%%x:%%x:%%x:%%x ]\n"
		"		[ msglvl %%d ] \n" },
    { "-a", "--show-pause", EEPROM_ENET_GPAUSE, "Show pause options" },
    { "-A", "--pause", EEPROM_ENET_SPAUSE, "Set pause options",
      "		[ autoneg on|off ]\n"
      "		[ rx on|off ]\n"
      "		[ tx on|off ]\n" },
    { "-c", "--show-coalesce", EEPROM_ENET_GCOALESCE, "Show coalesce options" },
    { "-C", "--coalesce", EEPROM_ENET_SCOALESCE, "Set coalesce options",
		"		[adaptive-rx on|off]\n"
		"		[adaptive-tx on|off]\n"
		"		[rx-usecs N]\n"
		"		[rx-frames N]\n"
		...
		"		[tx-frames-high N]\n"
	        "		[sample-interval N]\n" },
    { "-g", "--show-ring", EEPROM_ENET_GRING, "Query RX/TX ring parameters" },
    { "-G", "--set-ring", EEPROM_ENET_SRING, "Set RX/TX ring parameters",
		"		[ rx N ]\n"
		"		[ rx-mini N ]\n"
		"		[ rx-jumbo N ]\n"
	        "		[ tx N ]\n" },
    { "-k", "--show-offload", EEPROM_ENET_GOFFLOAD, "Get protocol offload information" },
    { "-K", "--offload", EEPROM_ENET_SOFFLOAD, "Set protocol offload",
		"		[ rx on|off ]\n"
		"		[ tx on|off ]\n"
		"		[ sg on|off ]\n"
	        "		[ tso on|off ]\n"
	        "		[ ufo on|off ]\n"
	        "		[ gso on|off ]\n" },
    { "-i", "--driver", EEPROM_ENET_GDRV, "Show driver information" },
    { "-d", "--register-dump", EEPROM_ENET_GREGS, "Do a register dump",
		"		[ raw on|off ]\n"
		"		[ file FILENAME ]\n" },
    { "-e", "--eeprom-dump", EEPROM_ENET_GEEPROM, "Do a EEPROM dump",
		"		[ raw on|off ]\n"
		"		[ offset N ]\n"
		"		[ length N ]\n" },
    { "-E", "--change-eeprom", EEPROM_ENET_SEEPROM, "Change bytes in device EEPROM",
		"		[ magic N ]\n"
		"		[ offset N ]\n"
		"		[ value N ]\n" },
    { "-r", "--negotiate", EEPROM_ENET_NWAY_RST, "Restart N-WAY negotation" },
    { "-p", "--identify", EEPROM_ENET_PHYS_ID, "Show visible port identification (e.g. blinking)",
                "               [ TIME-IN-SECONDS ]\n" },
    { "-t", "--test", EEPROM_ENET_TEST, "Execute adapter self test",
                "               [ online | offline ]\n" },
    { "-S", "--statistics", EEPROM_ENET_GSTATS, "Show adapter statistics" },
    { "-h", "--help", EEPROM_ENET_HELP, "Show this help" },
    {}
};
#endif
...
static void parse_cmdline(int argc, char **argp)
{
	...
	for (i = 1; i < argc; i++) {
		...
		case 1:
			...
			break;
		case 2:
			if ((mode & EEPROM_ENET_SSET) ||
			    (mode & EEPROM_ENET_GDRV) ||
			    (mode & EEPROM_ENET_GREGS)||
			    (mode & EEPROM_ENET_NWAY_RST) ||
			    (mode & EEPROM_ENET_TEST) ||
			    (mode & EEPROM_ENET_GEEPROM) ||
			    (mode & EEPROM_ENET_SEEPROM) ||
			    (mode & EEPROM_ENET_GPAUSE) ||
			    (mode & EEPROM_ENET_SPAUSE) ||
			    (mode & EEPROM_ENET_GCOALESCE) ||
			    (mode & EEPROM_ENET_SCOALESCE) ||
			    (mode & EEPROM_ENET_GRING) ||
			    (mode & EEPROM_ENET_SRING) ||
			    (mode & EEPROM_ENET_GOFFLOAD) ||
			    (mode & EEPROM_ENET_SOFFLOAD) ||
			    (mode & EEPROM_ENET_GSTATS) ||
			    (mode & EEPROM_ENET_PHYS_ID)) {
				devname = argp[i];
				break;
			}
			/* fallthrough */
		case 3:
			if (mode & EEPROM_ENET_TEST) {
				...
				break;
			} else if (mode & EEPROM_ENET_PHYS_ID) {
...
static int dump_regs(struct ethtool_drvinfo *info, struct ethtool_regs *regs)
{
	...
	if (gregs_dump_file) {
		...
		regs = mosys_realloc(regs, sizeof(*regs) + st.st_size);
		regs->len = st.st_size;
...
static int dump_eeprom(struct platform_intf *intf,
	...
	if (!strncmp("tg3", info->driver, ETHTOOL_BUSINFO_LEN)) {
		return tg3_dump_eeprom(intf, info, ifr, ee);
	}
...
int eeprom_enet_doit(struct platform_intf *intf,
		const char *devname, unsigned int mode)
{
	...
	if (fd < 0) {
		lprintf(LOG_ERR, "%s: Cannot get control socket", __func__);
		return -1;
	}

	/* all of these are expected to populate ifr->ifr_data as needed */
	if (mode & EEPROM_ENET_GEEPROM) {
		return do_geeprom(intf, fd, &ifr);
	/* original if-else statement */
//	if (mode & EEPROM_ENET_GDRV) {
//		return do_gdrv(fd, &ifr);
//	} else if (mode & EEPROM_ENET_GSET) {
//		return do_gset(fd, &ifr);
//	} else if (mode & EEPROM_ENET_SSET) {
//		return do_sset(fd, &ifr);
//	} else if (mode & EEPROM_ENET_GREGS) {
//		return do_gregs(fd, &ifr);
//	} else if (mode & EEPROM_ENET_NWAY_RST) {
//		return do_nway_rst(fd, &ifr);
//	} else if (mode & EEPROM_ENET_GEEPROM) {
//		return do_geeprom(fd, &ifr);
//	} else if (mode & EEPROM_ENET_SEEPROM) {
//		return do_seeprom(fd, &ifr);
//	} else if (mode & EEPROM_ENET_TEST) {
//		return do_test(fd, &ifr);
//	} else if (mode & EEPROM_ENET_PHYS_ID) {
//		return do_phys_id(fd, &ifr);
//	} else if (mode & EEPROM_ENET_GPAUSE) {
//		return do_gpause(fd, &ifr);
//	} else if (mode & EEPROM_ENET_SPAUSE) {
//		return do_spause(fd, &ifr);
//	} else if (mode & EEPROM_ENET_GCOALESCE) {
//		return do_gcoalesce(fd, &ifr);
//	} else if (mode & EEPROM_ENET_SCOALESCE) {
//		return do_scoalesce(fd, &ifr);
//	} else if (mode & EEPROM_ENET_GRING) {
//		return do_gring(fd, &ifr);
//	} else if (mode & EEPROM_ENET_SRING) {
//		return do_sring(fd, &ifr);
//	} else if (mode & EEPROM_ENET_GOFFLOAD) {
//		return do_goffload(fd, &ifr);
//	} else if (mode & EEPROM_ENET_SOFFLOAD) {
//		return do_soffload(fd, &ifr);
//	} else if (mode & EEPROM_ENET_GSTATS) {
...
static int do_gregs(int fd, struct ifreq *ifr)
{
	...
	regs = mosys_calloc(1, sizeof(*regs)+drvinfo.regdump_len);
...
static int do_geeprom(struct platform_intf *intf, int fd, struct ifreq *ifr) {
	...
	err = ioctl(fd, SIOCETHTOOL, ifr);
	if (err < 0) {
		lprintf(LOG_ERR, "%s: Cannot get driver information", __func__);
		return -1;
	}
	...
	eeprom = mosys_calloc(1, sizeof(*eeprom)+geeprom_length);
	...
	err = ioctl(fd, SIOCETHTOOL, ifr);
	if (err < 0) {
		lprintf(LOG_ERR, "%s: Cannot get EEPROM data", __func__);
		free(eeprom);
		return -1;
	}

	err = dump_eeprom(intf, &drvinfo, ifr, eeprom);
	free(eeprom);
...
static int do_test(int fd, struct ifreq *ifr)
{
	...
	test = mosys_calloc(1, sizeof(*test) + drvinfo.testinfo_len * sizeof(u64));
	...
	strings = mosys_calloc(1, sizeof(*strings) +
...
static int do_gstats(int fd, struct ifreq *ifr)
{
	...
	strings = mosys_calloc(1, sz_str + sizeof(struct ethtool_gstrings));
	stats = mosys_calloc(1, sz_stats + sizeof(struct ethtool_stats));
	if (!strings || !stats) {
